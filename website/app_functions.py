...
def ValuePredictor(to_predict_list):
    if len(to_predict_list) == 15:     
        ...
        with open('./website/app_models/liver_model.pkl', 'rb') as f:
            liver_model = joblib.load(f)
        pred = liver_model.predict(np.array(to_predict_list).reshape(-1, len(to_predict_list)))
    elif len(to_predict_list) == 11:
        page = 'heart'
        with open('./website/app_models/heart_disease_model.sav', 'rb') as f:
            heart_model = joblib.load(f)
        ...
        page = 'diabete'
        with open('./website/app_models/diabetes_model.sav', 'rb') as f:
